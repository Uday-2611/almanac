...
- Keep movie search status-aware in both result rows and provider-information previews. Show whether a title is not saved, in Watchlist, or Watched. The `Watched` action must open rating, watched-date, and review entry before saving directly to Watched; promote an existing Watchlist title without duplication and do not require an unsaved title to pass through Watchlist first.
