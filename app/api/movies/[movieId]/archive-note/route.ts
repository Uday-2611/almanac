...
import { archiveNoteSchema, idSchema, validationErrorMessage } from "@/lib/validation";
...
export async function PATCH(request: Request, { params }: Context) {
  ...
  if (!parsed.success) return Response.json({ error: validationErrorMessage(parsed.error, "Enter a valid Archive Note.") }, { status: 400 });
