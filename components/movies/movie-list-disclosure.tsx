...
export function MovieListDisclosure({
  ...
  return (
    <section aria-label={title}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-[#686868] lg:mt-0 lg:justify-end">
          ...
          {isConfirmingDelete ? (
            <>
              ...
              <button type="button" disabled={isPending} onClick={() => setIsConfirmingDelete(false)} className="ledger-focus underline underline-offset-4">Cancel</button>
            </>
          ) : null}
          <button
            type="button"
      ...
      <div
        id={`${id}-contents`}
        className={`grid transition-[grid-template-rows,opacity] duration-150 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}
      >
