...
export function ArchiveNotePage({ backHref, backLabel, creator, entryId, entryType, kind, note, reusableTags, tags, title, year }: ArchiveNotePageProps) {
  return (
    <main className="min-h-screen bg-white px-4 py-8 text-[#111111] sm:px-8 sm:py-12">
      <nav className="mx-auto mb-10 flex w-full max-w-[794px] items-center justify-between gap-4 text-xs text-[#686868]">
        <Link href={backHref} className="ledger-focus min-w-0 truncate py-2 hover:text-[#111111]">← {backLabel}</Link>
        <span className="shrink-0">Archive note</span>
      </nav>
      ...
      <article className="mx-auto min-h-[calc(100dvh-8rem)] w-full max-w-[794px] px-1 sm:px-8">
        <header className="pb-12">
          ...
          <h1 className="mt-7 break-words text-[clamp(2.5rem,13vw,3rem)] font-semibold leading-[0.94] tracking-[-0.055em] sm:text-7xl">{title}</h1>
