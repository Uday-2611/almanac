...
export function SearchResultSkeleton({ label = "Searching" }: { label?: string }) {
  return (
    <div role="status" aria-label={label} className="space-y-1">
      {[0, 1, 2, 3].map((item) => (
        <div key={item} className="grid min-h-[84px] grid-cols-[45px_minmax(0,1fr)] items-center gap-2 rounded-[4px] bg-white px-1.5 py-1.5 sm:grid-cols-[45px_minmax(0,1fr)_6rem]">
          ...
          <span className="col-start-2 h-2 w-20 animate-pulse justify-self-end bg-[#eeeeee] [animation-delay:240ms] sm:col-start-auto" />
        </div>
      ))}
    </div>
  );
