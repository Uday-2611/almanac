...
## Almanac private archive

---

## 1. Vision

A private, single-user-per-account archive for movies, TV shows, books, and standalone Texts notes. No feeds, no followers, no likes, no comments, no public profiles. The entire product is "you, quietly keeping records." Visual language: minimal, editorial, typographic — matching the reference screenshot (wordmark, hairline dividers, plain list rows, restrained UI chrome, generous whitespace).
...
- Typography: two-tier hierarchy per row (bold title, muted metadata line) is the core repeating pattern — reuse this row pattern for movies and books.
...
4. **Flexible media browsing:** `List view` presents a text ledger, `Image view` presents a horizontal artwork rail, and `Canvas view` presents a repeating, pannable artwork field.
...
Users can write private standalone notes with optional titles, editable journal dates, Markdown bodies, automatic saving, and optional many-to-many folders. Texts remain separate from movie and book Archive Notes.

### 3.4 View Modes (applies to Movies and Books)

- `List view`: plain rows, text-only, dense, sorted by date added/watched (most recent first) by default.
- `Image view`: horizontal poster or cover rail, same sort order, lazy-loaded.
- `Canvas view`: an image-only, two-dimensional field that repeats the current collection as the user pans.
...
/texts                  → Standalone private journal
/settings               → account and migration tools
...
**Phase 1 — Core logging (movies + books)**
...
**Phase 2 — Texts**
- Standalone private notes with automatic saving, local draft recovery, and folders.
...
- Treat the reference screenshot's row pattern (date-left / bold-title + gray-subline / hairline divider) as the base component (`ListRow`) reused across movies and books — build it once, generic over a small props shape (`{ leftMeta, title, subline, thumbnail?, onClick }`).
